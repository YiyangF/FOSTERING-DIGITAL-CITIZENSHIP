    ...
    <div class="container">
        ...
        <div v-if="analyzedResult" class="analyzed-message">
            ...
            <div class="result-content">
                ...
                <p class="result-percentage">Confidence: {{ (analyzedResult.percentage * 100).toFixed(2) }}%</p>
                <div class="chart-wrapper" v-if="getChartData">
                    <Pie :data="getChartData" :options="chartOptions" />
                </div>
                <!-- <div class="explanation" v-if="isCyberbullying">
                    <h4>Why this might be harmful:</h4>
                    <p>This message contains language that could be perceived as hurtful or offensive. Words like these can impact someone's self-esteem and emotional well-being.</p>
                    <h4>Suggestion:</h4>
                    <p>Consider using more supportive and constructive language. If you have concerns, express them respectfully and with empathy.</p>
                </div>
                <div class="explanation" v-else>
                    <h4>Assessment:</h4>
                    <p>This message appears to use appropriate and respectful language. Positive communication helps build healthy relationships.</p>
                </div> -->
            </div>
        </div>
...
export default {
  ...
  setup() {
    ...
    const isCyberbullying = computed(() => {
      ...
      return analyzedResult.value.result === 'cyberbully';
    });
    ...
    const detectMessage = async () => {
      ...
      try {
        const apiUrl = 'https://cyberbullying-ml-api-production.up.railway.app/predict';
        ...
        console.warn('API failed, using fallback analysis:', err.message);
        
        // Simple fallback analysis based on keywords
        const negativeWords = ['bad', 'hate', 'stupid', 'ugly', 'dumb', 'idiot', 'loser', 
                              'fat', 'pathetic', 'kill', 'die', 'shut up', 'useless'];
        
        const containsNegativeWords = negativeWords.some(word => 
          message.value.toLowerCase().includes(word.toLowerCase())
        );
        
        const isBullying = containsNegativeWords;
        const confidence = isBullying ? 0.7 + (Math.random() * 0.25) : 0.2 + (Math.random() * 0.3);
        
        analyzedResult.value = {
          result: isBullying ? 'cyberbully' : 'not cyberbully',
          percentage: confidence
        };
        
        // Don't show error to user, but keep a quiet warning in the console
    ...
    const getChartData = computed(() => {
        ...
        const confidence = analyzedResult.value.percentage * 100;
